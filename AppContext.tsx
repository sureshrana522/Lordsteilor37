...
export const AppProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const approveRequest = useCallback(async (reqId: string, approved: boolean) => {
    ...
    if (!req) return false;
    if (approved) {
        if (req.type === 'ADD_FUNDS') await addTransaction(req.userId, req.amount, 'Credit', 'Booking', `Admin Approved Fund Deposit`);
        else if (req.type === 'WITHDRAW') await addTransaction(req.userId, req.amount, 'Debit', 'Daily', `Admin Approved Withdrawal`);
    }
    if (isDemoMode) { setRequests(prev => prev.map(r => r.id === reqId ? { ...r, status } : r)); return true; } 
    else { try { await updateDoc(doc(db!, 'requests', reqId), { status }); return true; } catch (e) { return false; } }
  ...
  const seedDatabase = useCallback(async () => { 
      ...
      try { 
          ...
          const initialRates: StitchingRate[] = [
              { id: '1', type: 'Measurement', normal: 5, medium: 7, regular: 10, vip: 15, rateType: 'Percentage' },
              { id: '2', type: 'Cutting', normal: 10, medium: 12, regular: 15, vip: 20, rateType: 'Percentage' },
              { id: '3', type: 'Shirt Maker', normal: 20, medium: 25, regular: 30, vip: 40, rateType: 'Percentage' },
              { id: '4', type: 'Pant Maker', normal: 20, medium: 25, regular: 30, vip: 40, rateType: 'Percentage' },
              { id: '5', type: 'Delivery', normal: 2, medium: 3, regular: 5, vip: 8, rateType: 'Percentage' }
          ];
  ...
  const resetDatabase = useCallback(async () => {
    ...
    const collections = ['orders', 'customers', 'transactions', 'requests', 'system_logs'];
  ...
  const value = useMemo(() => ({ role, setRole, loginUser, authenticateUser, currentUser, registerUser: (u: any) => {}, autoRegister, orders, customers, materials, galleryItems, systemUsers, logs, stats: liveStats, investment, config, rates, stitchingRates, transactions, requests, updateOrderStage, addCustomer, addOrder: (o: any) => setOrders(p => [...p, o]), addMaterial: (m: any) => {}, moveOrder, confirmHandover, saveMeasurements, deleteOrder: (id: any) => {}, investFunds: async (a: any, p: any) => true, withdrawReturns: async (a: any) => true, transferFunds: async (r: any, a: any) => true, updateConfig, updateRates: (r: any) => {}, updateStitchingRates, addGalleryItem: (i: any) => {}, deleteGalleryItem: (id: any) => {}, updateSystemUser: (id: any, u: any) => {}, resetSystemIDs: () => {}, resetDatabase, addFrequentWorker: (id: any) => {}, getDashboardStats: () => liveStats, getWalletHistory: (t: any) => transactions.filter(tx => tx.userId === currentUser?.id && (t === 'Total' || tx.walletType === t)), requestAddFunds, requestWithdrawal, requestJoin: (n: any, m: any, r: any, u: any) => {}, approveRequest, releaseFundsManually: async (u: any, a: any, w: any, d: any) => true, addLog, seedDatabase, isDemoMode }), [role, currentUser, orders, customers, materials, galleryItems, systemUsers, logs, liveStats, investment, config, rates, stitchingRates, transactions, requests, loginUser, authenticateUser, autoRegister, updateOrderStage, addCustomer, moveOrder, confirmHandover, saveMeasurements, updateConfig, updateStitchingRates, approveRequest, addLog, seedDatabase, isDemoMode, resetDatabase, requestAddFunds, requestWithdrawal]);
